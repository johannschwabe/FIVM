IMPORT DTREE FROM FILE 'tpch_11-Q2.txt';
...
STREAM lineitem (
  ...
  FROM FILE './datasets/tpch10_sorted/lineitem.csv'
...
STREAM supplier (
  ...
  FROM FILE './datasets/tpch10_sorted/part.csv'
  LINE DELIMITED CSV (delimiter := '|');
...
STREAM partsupp (
  ...
  FROM FILE './datasets/tpch10_sorted/partsupp.csv'
  LINE DELIMITED CSV (delimiter := '|');

SELECT SUM(
    [lift<0>: RingFactorizedRelation<[0, INT]>](suppkey) *
    [lift<1>: RingFactorizedRelation<[1, INT]>](partkey) *
    [lift<2>: RingFactorizedRelation<[2, DECIMAL, INT, INT]>](l_quantity, l_linenumber, orderkey) *
