...
void Application::on_end_processing(dbtoaster::data_t &data, bool print_result) {
  ...
  const auto &top_level_view = data.get_V_postcode_HSIRDT1();
  V_postcode_HSIRDT1_entry *r0 = top_level_view.head;
  while (r0 != nullptr) {
    auto ring = r0->__av;
    r0 = r0->nxt;
    for (auto &t0: ring.store) {
      auto &key_0 = t0.first;
            ...
            for (auto &t4: rel4.store) {
              ...
              for (auto &t5: rel5.store) {
                ...
                for (auto &t6: rel6.store) {
                  ...
                  auto combined_value =
                      payload_0 * payload_1 * payload_2 * payload_3 * payload_4 * payload_5 * payload_6;
                  output_size++;
