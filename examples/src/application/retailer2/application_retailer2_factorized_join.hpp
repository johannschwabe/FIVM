...
#include "application_retailer2_base.hpp"
...
void Application::on_end_processing(dbtoaster::data_t &data, bool print_result) {
  ...
  const auto &toplevel_view = data.get_V_locn_IWLC1();
  for (auto &t0: toplevel_view.store) {
    auto &key_0 = t0.first;
    auto payload_0 = t0.second;
    V_dateid_IW1_entry e1;
    const auto &rel1 = data.get_V_dateid_IW1().getValueOrDefault(e1.modify(std::get<0>(key_0)));
    for (auto &t1: rel1.store) {
      auto &key_1 = t1.first;
      auto &payload_1 = t1.second;
      V_ksn_I1_entry e2;
      const auto &rel2 = data.get_V_ksn_I1().getValueOrDefault(e2.modify(std::get<0>(key_0), std::get<0>(key_1)));
      for (auto &t2: rel2.store) {
        auto &key_2 = t2.first;
        auto &payload_2 = t2.second;
        V_rain_W1_entry e3;
        const auto &rel3 = data.get_V_rain_W1().getValueOrDefault(e3.modify(std::get<0>(key_0), std::get<0>(key_1)));
        for (auto &t3: rel3.store) {
          auto &key_3 = t3.first;
          auto &payload_3 = t3.second;
          V_zip_LC1_entry e4;
          const auto &rel4 = data.get_V_zip_LC1().getValueOrDefault(e4.modify(std::get<0>(key_0)));
          for (auto &t4: rel4.store) {
            auto &key_4 = t4.first;
            auto &payload_4 = t4.second;
            V_rgn_cd_L1_entry e5;
            const auto &rel5 = data.get_V_rgn_cd_L1().getValueOrDefault(
                e5.modify(std::get<0>(key_0), std::get<0>(key_4)));
            for (auto &t5: rel5.store) {
              auto &key_5 = t5.first;
              auto &payload_5 = t5.second;
              V_population_C1_entry e6;
              const auto &rel6 = data.get_V_population_C1().getValueOrDefault(e6.modify(std::get<0>(key_4)));
