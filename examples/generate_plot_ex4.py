...
x_ticks = []
x_tick_labels = []
for version_idx, dataset_version in enumerate(tpch_2['dataset'].unique()):
    dataset_data = tpch_2[tpch_2['dataset'] == dataset_version]
    ...
    for query_root_unique in sorted(dataset_data['query_root_unique'].unique()):
        ...
        for query in query_root_unique_data['query'].unique():
            ...
            for executor in query_data['executor'].unique():
                ...
                avg_height = exucutor_data["enumeration_time"].mean() / 1000
                std_height = exucutor_data["enumeration_time"].std() / 1000
...
ax.set_ylabel(f'Enumeration time (s)')
...
fig.suptitle("Enumeration times")
# plt.show()
plt.savefig(os.path.join(output_dir, f'Ex4_plot.png'), bbox_inches='tight', dpi=300)
